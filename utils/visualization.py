...
import os
import matplotlib
from distances import poincare_distance, euclidean_distance
...
def scatterplot_2d(latent_dist_fun, i, latent_embeddings, input_embeddings, CIDs,subjects=None, color_ENTROPY=False, shape_subject=False, args=None, save=False, plot_edges=False, losses=[],
                   losses_pos=[], losses_neg=[]):
    color_map = 'plasma'
    # latent_embeddings_norm = torch.norm(latent_embeddings, dim=-1)
    data_dist_matrix = scipy.spatial.distance.cdist(input_embeddings, input_embeddings, metric='hamming') * \
    ...
    if latent_dist_fun == 'euclidean':
        radius = euclidean_distance(torch.from_numpy(latent_embeddings), torch.zeros((1, 2)))
    elif latent_dist_fun == 'poincare':
    ...
    if latent_dist_fun == 'euclidean':
        plt.xlabel('Euclidean radius', fontsize=30)
    elif latent_dist_fun == 'poincare':
   ...
   # plt.title('Correlation between Radius and Entropy', fontsize=16)  # Added title
    ...
    # to visualize all subjects:
    # ax.scatter(latent_embeddings[:, 0], latent_embeddings[:, 1], c=np.linalg.norm(input_embeddings, axis=-1), cmap=color_map, s=300, zorder=10) 
    ax.scatter(latent_embeddings[:, 0], latent_embeddings[:, 1], c=c, cmap=color_map, s=300, zorder=10) 
    
    #To plot with a colorbar
    scatter = ax.scatter(latent_embeddings[:, 0], latent_embeddings[:, 1], c=c, cmap=color_map, s=300, zorder=10) 
    cbar = plt.colorbar(scatter, ax=ax, shrink=0.75)
    cbar.set_label('Entropy') 

    # #To visualize only one subject instead:
